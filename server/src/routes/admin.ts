...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const [users, establishments, reviews, pendingReports, pendingClaims] =
      await Promise.all([
        prisma.user.count(),
    ...
    const byStatus = await prisma.establishment.groupBy({
      by: ["status"],
      _count: true,
    });
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (claim.status !== "PENDING") {
      res.status(400).json({ error: "este claim ya fue procesado" });
      return;
    }
    ...
    // si se aprueba, asignar ownership y cambiar rol del usuario
